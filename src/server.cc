...
void MessageQueueServer::run()
{
  ...
  // rpc exchange
  channel->declareExchange(this->rpc_exchange_n, AMQP::topic, AMQP::durable);
  // event exchange
  channel->declareExchange(this->event_exchange_n, AMQP::topic, AMQP::durable);
  ...
  {
    channel->declareQueue(this->rpc_queue_n, AMQP::exclusive+AMQP::autodelete);
    channel->bindQueue(this->rpc_exchange_n, this->rpc_queue_n, this->rpc_queue_n);
    channel->consume(this->rpc_queue_n)
      .onReceived(
        [this](const AMQP::Message &message, uint64_t tag, bool) {
          auto req_id = message.correlationID();
          auto reply_to = message.replyTo();
          _on_rpc_message(message, req_id, reply_to);
          channel->ack(tag);
        }
      );
  }
  ...
  {
    channel->declareQueue(this->event_queue_n, AMQP::durable);
    channel->bindQueue(this->event_exchange_n, this->event_queue_n, this->event_queue_n);
    channel->consume(this->event_queue_n, AMQP::noack)
      .onReceived(
...
void MessageQueueServer::_on_rpc_message(const AMQP::Message &message, const string &req_id, const string &reply_to) {
  pool.start([&message, req_id, reply_to, this]{_rpc_worker(message, req_id, reply_to);});
}
...
void MessageQueueServer::_on_event_message(const AMQP::Message &message)
{
  ...
  {
    pool.start([cb_id, event_type, event_data, this]{_event_worker(event_type, cb_id, event_data);});
  }
}
...
void MessageQueueServer::_rpc_worker(const AMQP::Message &message, const string &req_id, const string &reply_to) {
  string body = message.body();
  auto body_vec = json_loadb(body.substr(0, message.bodySize()));
  string id{""};
  string meth{""};
...
{
  json res_json;
  res_json.push_back(res->type);
  res_json.push_back(res->msgid);
  res_json.push_back(res->error);
  auto res_data = json::parse(res->result);
  res_json.push_back(res_data[0]);
  auto body = res_json.dump();
  AMQP::Envelope env(body);
  env.setCorrelationID(res->msgid);

  channel->publish("", res->reply_to, env);
