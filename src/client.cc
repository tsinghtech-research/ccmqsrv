...
std::tuple<string, string>&& AsyncResult::get(int timeout) {
  std::unique_lock<std::mutex> lock(mutex_);
  if (!done_) {
...
void MessageQueueClient::on_response(const AMQP::Message &message) {
  ...
  if (req_events.find(req_id) != req_events.end()) {
    ...
    req_events[req_id]->set(std::tuple<string, string>(error, result));
  }
}
...
void MessageQueueClient::run() {
  ...
  recv_channel->declareExchange(rpc_exchange_, AMQP::topic, AMQP::durable);
  recv_channel->declareExchange(event_exchange_, AMQP::topic, AMQP::durable);

  recv_channel->declareQueue(callback_queue_, AMQP::exclusive+AMQP::autodelete);
  recv_channel->bindQueue(rpc_exchange_, callback_queue_, callback_queue_)
    .onSuccess([&]{
      is_running = true;
    });
  recv_channel->consume(callback_queue_, AMQP::noack)
    .onReceived(
...
void MessageQueueClient::release() {
  ...
  recv_channel->close();
  send_channel->close();
  send_channel = nullptr;
  recv_channel = nullptr;
}
